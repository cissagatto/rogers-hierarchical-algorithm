...
computeRogers <- function(ds,
  ...
  s = 1
  rogersParalel <- foreach(s = 1:number_folds) %dopar%{
...
CutreeHClust <- function(ds,
  ...
  cutreeParalel <- foreach(s = 1:number_folds) %dopar%{
    ...
    wardD = c(0)
    wardD2 = c(0)
    c4 = data.frame(fold, average, complete, mcquitty, 
                    single, wardD, wardD2)
    ...
    num.group = c(0)
    frequencia = c(0)
    final = data.frame(num.fold, num.part, num.group, 
                       frequencia, metodo.name)
    ...
    num.part = c(0)
    num.group = c(0)
    resumePartitions = data.frame(num.fold, num.part, 
                                  num.group, metodo.name)
    
    #####################################################################
    # methods
    metodos = c("average", "complete", "mcquitty", 
    ...
    for(i in i:length(metodos)){
      ...
      print(plot(as.phylo(HC), type = "radial",
